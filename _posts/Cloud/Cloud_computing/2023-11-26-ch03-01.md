...
KVM은 리눅스 커널에 KVM 모듈이 설치되면서 리눅스 자체가 타입 1 하이퍼바이저가 되는 구조이고 I/O 에뮬레이션은 QEMU(quick emulator)을 사용한다 <br>

에뮬레이션은 이기종 하드웨어에서 동작하는 하드웨어 환경을 구현하는 것으로 예를 들어 x86 환경에서 ARM 기반의 소프트웨어를 동작하려고 할 때 x86 시스템이 ARM 명령을 이해하고 x86 명령으로 변경해야 하느데 이 역할을 하는 것이 에뮬레이터이다.
